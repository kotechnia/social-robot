    ...
    while True:
        ...
        for dt in tqdm(train_loader, disable=opt.disable_tqdm):
            ...
            if iteration % losses_log_every == 0:
                ...
                tqdm.write('\n')
                tqdm.write(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
                tqdm.write(
        ...
        if (epoch % opt.save_checkpoint_every == 0) and (epoch >= opt.min_epoch_when_save):
            ...
            tqdm.write('Save model at iter {} to {}.'.format(iteration, checkpoint_path))
            ...
            if current_score >= best_val_score:
                ...
                tqdm.write('Save Best-model at iter {} to checkpoint file.'.format(iteration))
            ...
            tqdm.write('Save info to info.json')
